...
def GD (f, df, X_old, maxit, tol_f, tol_x):
    ...
    fun_history = np.zeros((maxit + 1,2))
    fun_history[0,]=X_old
    ...
    while k < maxit and current_grad_norm > tol_f:
        # Direzione di discesa
        p_k=-df(X_old)
        # Si chiama il metodo del gradiente perchè la direzione di discesa = -gradiente
    ...
    if k<maxit:
        fun_history = fun_history[:k+1]

    return X_old, k, fun_history, exit_flag
...
ris = GD(f, df, punto, maxit, tolleranza_f, tolleranza_x)
soluzione = ris[0]
n_iterazione = ris[1]
path_history = ris[2]
